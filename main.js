...
ScrollReveal().reveal('.professional-skill .skill-main-down', {origin: 'right'});
 /*-----------------------type-------------------*/
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    skillBars.forEach(skillBar => {
        // Get the percentage from the <p> with class 'percent'
        const percentText = skillBar.querySelector('.percent').textContent;
        const percentValue = percentText.replace('%', ''); // Remove the '%' character

        // Set the width of the corresponding span
        const barSpan = skillBar.querySelector('span');
        barSpan.style.width = percentText; // Set width to percentage
    });
});
